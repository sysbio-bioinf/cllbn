...
freqInRichterPopulation <- freqInRichterPopulation[markerGenes[!grepl("NOTCH3", markerGenes)],]
...
pdf("output/harmonizedUMAP.pdf", paper = "a4r")
...
pdf("output/barplotPopulationFrequencyInactive.pdf", paper = "a4r")
ggplot(frequencyTab[frequencyTab$State == "inactive",], aes(fill=Source, y = Frequency, x = Gene)) + geom_bar(position="dodge", stat="identity") + theme(axis.text.x = element_text(angle = 50, vjust = 0.5, hjust=1))
dev.off()
